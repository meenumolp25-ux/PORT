...
function initJournalModal() {
    ...
    if (!modal) {
        ...
        if (!document.getElementById('journal-modal-style')) {
            ...
            style.textContent = `
                .modal { position: fixed; inset: 0; display: none; align-items: center; justify-content: center; z-index: 2147483640; }
                .modal.show { display: flex; }
                .modal-backdrop { position: absolute; inset: 0; background: rgba(0,0,0,0.45); z-index: 2147483638; }
                .modal-panel { position: relative; background: var(--bg-card, #fff); color: var(--text, #111); max-width: 760px; width: 94%; border-radius: 12px; padding: 1.25rem; box-shadow: 0 10px 30px rgba(0,0,0,0.2); z-index: 2147483641; pointer-events: auto; }
